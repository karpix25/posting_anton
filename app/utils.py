...
def extract_theme(path: str, aliases: dict = None) -> str:
    parts = [p for p in path.replace("\\", "/").split("/") if p and p != "disk:"]
    v_idx = -1
    for i, p in enumerate(parts):
        if p.lower() in ["video", "видео"]:
    ...
    return normalize_theme_key(theme_raw, aliases) if theme_raw else "unknown"


def normalize_theme_key(text: str, aliases: dict = None) -> str:
    raw = normalize(text)
    
    # Use provided aliases or fallback to legacy config (cached)
    if aliases is None:
        try:
